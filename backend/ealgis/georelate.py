...
class SnappedGeometry(object):
    ...
    def make_gridded_temp(self, source, srid):
        ...
        table = eal.db.Table(table_name, metadata, *cols, prefixes=[])
        metadata.create_all(eal.db.engine)
        eal.db.session.commit()

        eal.db.session.execute(sqlalchemy.func.addgeometrycolumn(
                    table_name,
        ...
        eal.db.session.execute("CREATE INDEX %s ON %s USING gist ( %s )" % (\
            "%s_%s_gist" % (
                table_name,
                "the_geom"),
            table_name,
...
class Relate(object):
    ...
    def area(self, gid):
        return eal.db.session.query(st_area(self.gridded.get_geom_attr())).filter(self.gridded.get_gid_attr()==gid).one()[0]

    def gid_subquery(self, gid, lbl):
        return eal.db.session.query(self.gridded.get_geom_attr().label(lbl)).filter(self.gridded.get_gid_attr()==gid).subquery()
...
def find_intersections(left, right, left_relation, right_relation):
    ...
    def add_intersection(*args):
        ...
        sys.stderr.flush()
        # print len(intersections), args[1:]
...
def build_relations(left_source, right_source):
    def add_relation(f, t):
        ...
        if existing is not None:
            ...
            eal.db.session.query(GeometryIntersection).filter(GeometryIntersection.geometry_relation_id==existing.id).delete()
            eal.db.session.query(GeometryTouches).filter(GeometryTouches.geometry_relation_id==existing.id).delete()
            eal.db.session.commit()
            return existing
